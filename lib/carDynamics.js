export class CarDynamics {
  ...
  calculateTemperature(speedMs, rpm, currentTemp, deltaTime, engineRunning) {
    ...
    // Engine running: heat from RPM + throttle, cooled by airflow + radiator
    ...
    const heatIn = (rpmHeat + loadHeat) * deltaTime;
    // Airflow cooling scales with speed; at idle only radiator fan works
    const airCool = speedMs > 2 ? Math.min(speedMs * 0.055, 6) : 1.0; // fan at idle
  ...
  analyzePerformance(speedMs, rpm, temperature, currentGear) {
    ...
    if (gearEff < 0.6) {
      recs.push({
        ...
        message: `Gear efficiency ${Math.round(gearEff * 100)}%. Shift to a more suitable gear.`
      });
    }
    if (temperature > this.performanceMetrics.optimalTempRange[1]) {
      ...
      recs.push({
        type: "temperature", severity: "high",
        message: `Engine temp ${Math.round(temperature)}°C — exceeds safe range. Reduce load.`
      });
    } else if (temperature < this.performanceMetrics.optimalTempRange[0]) {
      recs.push({
        type: "temperature", severity: "medium",
        message: "Engine warming up — performance limited until optimal temperature."
      });
    }
    const [rpmLo, rpmHi] = this.performanceMetrics.optimalRpmRange;
    if (rpm > rpmHi) {
      const penalty = Math.min(15, Math.round(((rpm - rpmHi) / 1000) * 8));
      score = Math.max(0, score - penalty);
      recs.push({
        type: "rpm", severity: "medium",
        message: `RPM ${Math.round(rpm)} above optimal band. Upshift for efficiency.`
      });
    } else if (rpm < rpmLo && speedMs > 5) {
      recs.push({
        type: "rpm", severity: "low",
        message: `RPM ${Math.round(rpm)} below optimal band. Downshift for better response.`
      });
    }
    if (hp > 400 && throttle > 0.85) {
      recs.push({
        type: "power", severity: "low",
        message: `High power demand (${Math.round(hp)} HP). Moderate throttle for fuel savings.`
      });
    }
